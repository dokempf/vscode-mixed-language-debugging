...
* C++ source files:
  * `include/cross-language-debugging/cross-language-debugging.hpp` is the main
    C++ header that declares the interface of your library.
  * `src/cross-language-debugging.cpp` is the main file that implements this library.
  * `app/cross-language-debugging_app.cpp` is an executable that uses the library.
    This can e.g. be used to provide a command line interface for your project.
  * `tests/cross-language-debugging_t.cpp` contains the unit tests for the library.
  ...
  * The `python/crosslanguagedebugging` directory contains a Python
    package for the project. It contains a compiled Python module `_crosslanguagedebugging`
    that CMake generates from the Pybind11 source file `python/crosslanguagedebugging/_cross-language-debugging.cpp`. Additionally, it contains pure Python sources (e.g. `__init__.py`) that
